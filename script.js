...
async function login() {
    ...
    try{
        ...
        for(let i=1;i<rows.length;i++){
            let r=rows[i];
            if(r[2]==admission){
                let amount=parseFloat(r[5])||0; totalPaid+=amount;
                table+=`<tr>
...
<div><b>Amount:</b> ₹${amount}</div>
        ...
        let bal=document.getElementById("feeBalance"); bal.innerText="₹"+feeBalance;
...
document.addEventListener("DOMContentLoaded",()=>{ document.getElementById("loginBtn").addEventListener("click",login); });
